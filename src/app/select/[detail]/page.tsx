...
import { PiAlarmLight, PiCarProfileLight, PiPhoneLight, PiTrainLight, PiMapPinLight, PiMoneyLight, PiInfoLight } from "react-icons/pi";
    ...
    <div className="detail-page">
      ...
      <main>
        ...
        <section className="detail-description">
          ...
          <h3><PiMapPinLight className="detail-icon" />所在地</h3>
          <div>
            <div className="post-code">{`〒${cards[index].postCode}`}</div>
            <p className="detail-address"><a href={`https://www.google.co.jp/maps/search/${cards[index].title}`}>{cards[index].address}</a></p>
          </div>
          <h3><PiMoneyLight className="detail-icon" />料金</h3>
          <p>{cards[index].price}</p>
          <h3><PiAlarmLight className="detail-icon" />営業日・時間</h3>
          <p>{cards[index].business}</p>
          <h3><PiPhoneLight className="detail-icon" />電話番号</h3>
          <p>{cards[index].phoneNumber}</p>
          <h3><TbBrandProducthunt className="detail-icon" />駐車場</h3>
          <p>{cards[index].parking}</p>
          <h3><BiMaleFemale className="detail-icon" />トイレ</h3>
          <p>{cards[index].toilet}</p>
          <h3><PiInfoLight className="detail-icon" />定休日</h3>
          <p>{cards[index].closed}</p>
          <h3><PiTrainLight className="detail-icon" />公共交通機関でのアクセス</h3>
          <div>
            <ul>
          ...
          <h3><PiCarProfileLight className="detail-icon" />車でのアクセス</h3>
          <div>
            <ul>
      ...
      <footer className="detail-footer">
        <Link href="/select">
          <div
            className="detail-footer-dislike"
            onClick={() => handleClickDislike(index)}
          >
